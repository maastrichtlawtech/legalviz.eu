...
function registerApiRoutes(app, deps) {
  ...
  app.get('/api/laws/:celex/procedure', rateLimitMiddleware, async (req, res) => {
    try {
      const { celex } = req.params;
      ...
      const payload = await fetchLegislativeProcedure(celex, runSparqlQuery);
      cacheSet(resolutionCache, cacheKey, payload, RESOLUTION_CACHE_MS);
      res.json(payload);
    } catch (err) {
      safeErrorResponse(res, err, 'Failed to fetch legislative procedure');
    }
  });
  ...
  app.get('/', (req, res) => {
    res.json({
      ...
      endpoints: {
        ...
        'POST /mcp': 'Model Context Protocol endpoint (stateless Streamable HTTP). Tools include search_eu_law, search_law_text, resolve, get_law_part, get_citing_provisions, get_case_law, and get_law_relations. Use search_law_text for words found inside law provisions. Add to an AI client, e.g. `claude mcp add --transport http eurlex <base-url>/mcp`'
      },
      celexExamples: {
        '32016R0679': 'GDPR',
        '32024R1689': 'AI Act',
