...
| --- | --- | --- | --- |
| Corpus | [`refresh-corpus.yml`](../../.github/workflows/refresh-corpus.yml) — `Refresh corpus` | Monthly schedule `43 3 5 * *`, or manual dispatch | Immutable `corpus-vN` release and raw-corpus candidate artifact |
| Data | [`refresh-data.yml`](../../.github/workflows/refresh-data.yml) — `Refresh data` | Successful `workflow_run` of `Refresh corpus`, or manual dispatch | Immutable `data-vN` release and `automation/data-vN` Docker PR |
| Full text | [`refresh-fulltext.yml`](../../.github/workflows/refresh-fulltext.yml) — `Refresh fulltext` | Successful `workflow_run` of `Refresh data`, or manual dispatch | Immutable `fulltext-vN` release and `automation/fulltext-vN` Docker PR |

The normal monthly chain is therefore:

```text
Refresh corpus (corpus-vN)
        -> successful workflow_run
Refresh data (data-vN + DATA_RELEASE_TAG PR)
        -> successful workflow_run
Refresh fulltext (fulltext-vN + FULLTEXT_RELEASE_TAG PR)
```
...
also exposes `publish: false` for candidate-only inspection. A manual run may
accept the next exact release tag where that workflow exposes `release_tag`;
...
still upload its validation artifact, but it does not advance `fulltext-vN`.
...
`Refresh corpus` restores the latest immutable `corpus-vN` and current search
cache, then updates all raw inputs used by later stages:
...
6. Package the three tar archives and diagnostics. Publication creates a new
   `corpus-vN` release; an existing published corpus tag is never overwritten.
...
restores the latest immutable corpus and current `data-vN` assets, then:
...
7. Publishes the next immutable `data-vN` release only after candidate
   validation, then opens `automation/data-vN`. That PR changes only
   `ARG DATA_RELEASE_TAG` in `backend/Dockerfile` and dispatches the backend
   Docker smoke/evaluation workflow for the exact branch.
...
the latest `data-vN`, `fulltext-vN`, and corpus `laws`/`laws-html` assets, then:
...
5. When `actCount` grows, publish the next immutable `fulltext-vN` release and
   open `automation/fulltext-vN`. That PR changes only
...
| Failure | Recovery |
| --- | --- |
| Corpus download, Cellar audit, or backfill fails | Inspect logs and `missing.txt`, then rerun `Refresh corpus` manually from the latest published `corpus-vN`. Do not edit a published corpus tag. |
...
| Data manifest/count validation fails | Do not publish `data-vN`. Compare source cache counts, SQLite manifest integrity, carried-forward hashes, and the upstream corpus release; retry `Refresh data` after correction. |
...
| A published release exists but its PR was interrupted | Rerun the same stage. It verifies the immutable release and recovers/updates the exact `automation/data-vN` or `automation/fulltext-vN` branch rather than overwriting release contents. |
