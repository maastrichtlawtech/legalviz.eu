 ...
 * implementing/delegated acts, a legislative procedure link and reverse
 * citations, keyed by CELEX. Shared by
...
export function useLawMetadata(celex) {
  ...
  useEffect(() => {
    ...
    fetchLegislativeProcedure(celex)
      .then((result) => {
        if (!cancelled) setProcedure(result?.procedureUrl ? result : null);
      })
      .catch(() => { if (!cancelled) setProcedure(null); });
