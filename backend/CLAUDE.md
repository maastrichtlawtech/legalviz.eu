...
- **Metadata sidecars** (`search/data/eurovoc.json`, `search/data/dates.json`): the release-asset cache doesn't carry EuroVoc subjects or a `work_date_document` for the ~67k older acts the corpus expansion added. `search/fetch-eurovoc.js` and `search/fetch-dates.js` fetch those from Cellar SPARQL into small **committed** CELEX-keyed sidecars that `legal-cache-store.js` merges at load time (`record.eurovoc`; `record.date` only when the cache has none). Both are resumable and read the current cache for their target CELEX — **regenerate them whenever you rebuild the cache**, or the new records silently serve empty `topics`/missing dates (this exact drift is how they went stale after the `data-v5` rebuild).
